...
def rep():
  ...
  if a[PId] >= 20 and a[PId] < 30:
    try:
      ...
      open(f'pgbot/{PId}/Docs.txt', "w").write("""<h1>Cannot view.</h1>
<p>The site is temporarily unavailable for viewing due to suspected violations of the terms and conditions.</p>
<p>Reason: REPORT</p>""")
  if a[PId] > 30:
    try:
...
def com1():
  ...
  try:
    ...
    f.write("<b>" + request.form.get("title") +
            f"</b><br>POST:<small>{usn}</small><br>")
    f.write(txt)
    f.write("\n\n<br><br>\n\n\n")
  except FileNotFoundError:
    try:
